...
**JULES SUBAGENT REFERENCE**: The Jules Terminal Client and subagent system is located in `[.agents/skills/jules-subagent/]`. This directory contains the client interface, configuration, and examples for delegating work to autonomous subagents.

**EXECUTION MODE REFERENCE**: When switching to execution mode, you will revert to the "Director" persona as defined in `[.agents/on-demand/director.md]`. This file contains your execution guidelines and workflow.
...
*   Follow the **Base 3-File Pattern** for every task: `task_plan.md`, `notes.md`, and `[deliverable].md` - all stored in `.agents/orchestrator_tasks/task-DD-MM-YYYY--HH-MM-SS/` subfolders.
...
*   **Create task folder:** First create a new folder `.agents/orchestrator_tasks/task-DD-MM-YYYY-HH-MM-SS/` using the current timestamp.
...
*   Execute the creation commands using the following prompt template:
    ```
    You are a "Director" as defined in `[.agents/on-demand/director.md]`. This file contains your execution guidelines and workflow.
...
1.  **Read:** Review `.agents/orchestrator_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` to ground your attention.
2.  **Act:** Write code, fetch data, or synthesize information for your specific chunk.
3.  **Store:** Append large outputs directly to `.agents/orchestrator_tasks/task-DD-MM-YYYY-HH-MM-SS/notes.md` or the deliverable file. Never modify previous context history; rely on append-only context generation.
4.  **Log:** If an action fails, document the failure trace directly in the `.agents/orchestrator_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` errors section so your internal understanding updates.
5.  **Edit:** Use file editing to update checkboxes (`[x]`) and statuses in `.agents/orchestrator_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` as you progress, rather than rewriting the whole file.
...
🎁 **PHASE 6: DELIVERY**
*   Present the final deliverable to the user only when all checkboxes in `.agents/orchestrator_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` are marked complete, and inform them of any `TODO` files that were generated during the process.