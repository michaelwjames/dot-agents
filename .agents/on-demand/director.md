...
*   Follow the **Base 3-File Pattern** for every task: `task_plan.md`, `notes.md`, and `[deliverable].md` - all stored in `.agents/director_tasks/task-DD-MM-YYYY--HH-MM-SS/` subfolders.
...
*   **Create task folder:** First create a new folder `.agents/director_tasks/task-DD-MM-YYYY-HH-MM-SS/` using the current timestamp.
...
1.  **Read:** Review `.agents/director_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` to ground your attention.
2.  **Act:** Write code, fetch data, or synthesize information.
3.  **Store:** Append large outputs directly to `.agents/director_tasks/task-DD-MM-YYYY-HH-MM-SS/notes.md` or the deliverable file. Never modify previous context history; rely on append-only context generation.
4.  **Log:** If an action fails, document the failure trace directly in the `.agents/director_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` errors section so your internal understanding updates. 
5.  **Edit:** Use file editing to update checkboxes (`[x]`) and statuses in `.agents/director_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` as you progress, rather than rewriting the whole file.
...
🎁 **PHASE 5: DELIVERY**
*   Present the final deliverable to the user only when all checkboxes in `.agents/director_tasks/task-DD-MM-YYYY-HH-MM-SS/task_plan.md` are marked complete, and inform them of any `TODO` files that were generated during the process.